  ...
  <div class="admin-articles">
    ...
    <el-card class="filter-card">
      <el-form :model="filterForm" inline>
        ...
        <el-form-item>
          <el-button type="primary" @click="loadArticles">筛选</el-button>
    ...
    <!-- 文章详情对话框 -->
    <el-dialog v-model="articleDetailVisible" title="文章详情" width="800px">
      <div v-if="selectedArticle" class="article-detail">
        <el-descriptions :column="2" border>
          <el-descriptions-item label="ID">{{ selectedArticle.id }}</el-descriptions-item>
          <el-descriptions-item label="标题">{{ selectedArticle.title }}</el-descriptions-item>
          <el-descriptions-item label="作者">{{ selectedArticle.authorName }}</el-descriptions-item>
          <el-descriptions-item label="分类">{{ selectedArticle.categoryName }}</el-descriptions-item>
          ...
          <el-descriptions-item label="特殊标记">
            <div>
              <el-tag v-if="selectedArticle.isPinned" type="danger" size="small">置顶</el-tag>
              <el-tag v-if="selectedArticle.isFeatured" type="warning" size="small">精选</el-tag>
            </div>
          </el-descriptions-item>
          <el-descriptions-item label="浏览量">{{ selectedArticle.viewCount }}</el-descriptions-item>
          <el-descriptions-item label="点赞数">{{ selectedArticle.likeCount }}</el-descriptions-item>
          <el-descriptions-item label="评论数">{{ selectedArticle.commentCount }}</el-descriptions-item>
          <el-descriptions-item label="创建时间">{{ formatDate(selectedArticle.createdAt) }}</el-descriptions-item>
          <el-descriptions-item label="更新时间">{{ formatDate(selectedArticle.updatedAt) }}</el-descriptions-item>
        </el-descriptions>
...
const handleAction = async (command, article) => {
  ...
  switch (command) {
    case 'approve':
      await approveArticleAction(article)
      break
    case 'reject':
      await rejectArticleAction(article)
      break
    case 'pin':
      await pinArticleAction(article)
      break
    case 'unpin':
      await unpinArticleAction(article)
      break
    case 'feature':
      await featureArticleAction(article)
      break
    case 'unfeature':
      await unfeatureArticleAction(article)
      break
    case 'delete':
      await deleteArticleAction(article)
      break
  }
}

const approveArticleAction = async (article) => {
  try {
    await ElMessageBox.confirm('确定要审核通过该文章吗？', '审核通过', {
...
const rejectArticleAction = async (article) => {
  try {
...
const pinArticleAction = async (article) => {
  try {
...
const unpinArticleAction = async (article) => {
  try {
...
const featureArticleAction = async (article) => {
  try {
...
const unfeatureArticleAction = async (article) => {
  try {
...
const deleteArticleAction = async (article) => {
  try {
    await ElMessageBox.confirm('确定要删除该文章吗？此操作不可恢复！', '删除文章', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
